...
\begin{document}
    ...
    \end{equation}
    пошаговым методом. Для оценки сходимости запишем равенство~\eqref{eq:approx_single_help} в точке $x^*$ и вычтем из
    него равенство~\eqref{eq:approx_single}:
    \begin{equation*}
        \varepsilon_{n+1} = x^* - x_{n+1} = \varphi(x^*) - \varphi(x_n) = \varphi(x_n + \varepsilon_n) - \varphi(x_n)
    \end{equation*}
    Раскладывая $\varphi(x_n + \varepsilon_n)$ в ряд по степеням $\varepsilon_n$ и ограничиваясь в остаточном члене
    первой производной, получаем уравнение погрешности
    \begin{equation*}
    ...
    Высокой скоростью сходимости в ряде случаев обладает \emph{метод Ньютона} (или \emph{метод касательных}).
    Подставляя в уравнение $f(x) = 0$ его корень $x^*$, и раскладывая в ряд по степеням $\varepsilon_n$
    \begin{equation*}
